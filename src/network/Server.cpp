...
bool Server::init() {
    ...
    if (flags < 0 || ::fcntl(_serverFd, F_SETFL, flags | O_NONBLOCK) < 0) {
        std::cerr << "Error: setting socket to non-blocking failed: " << std::strerror(errno) << std::endl;
        ::close(_serverFd);
        _serverFd = -1;
        return false;
    }
    ...
    if (::bind(_serverFd, (struct sockaddr*)&address, sizeof(address)) < 0) {
        std::cerr << "Error: bind to port " << _port << " failed: " << std::strerror(errno) << std::endl;
        ::close(_serverFd);
        _serverFd = -1;
        return false;
    }
...
void Server::run() {
    ...
    while (_running && !SignalHandler::shouldExit()) {
        ...
        if (activity < 0) {
            ...
            std::cerr << "Error: select failed: " << std::strerror(errno) << std::endl;
            break;
        }
